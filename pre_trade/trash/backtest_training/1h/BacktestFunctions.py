...
def get_zscore(training_df: pd.DataFrame, slope: float, period: int):
    training_df['Open Time'] = pd.to_datetime(
        training_df['Open Time']/1000, unit='s')
    training_df['n_log_price_chosen'] = slope*training_df['log_price_chosen']
    training_df['spread'] = training_df['log_price_compare'] - \
        training_df['n_log_price_chosen']
    training_df['mean_spread'] = training_df['spread'].rolling(
        period).mean().shift()  # shift to avoid look-ahead bias
    training_df['std_spread'] = training_df['spread'].rolling(
        period).std().shift()
    training_df['z-score'] = (training_df['spread'] -
                              training_df['mean_spread'])/training_df['std_spread']
    training_df.dropna(inplace=True)
    return training_df


def get_entry_chosen(cash_initial_chosen: float, cash_percentage: float, price_chosen: float, transaction_cost: float):
    cash_spent_chosen = cash_initial_chosen * \
        cash_percentage  # calculate money spent
    # calculate the amount of bought coins
    amount_chosen = cash_spent_chosen/price_chosen
    commission_chosen = cash_spent_chosen * \
        transaction_cost  # calculate commission fee
    cash_initial_chosen = cash_initial_chosen - \
        commission_chosen  # calculate the remain cash
    return cash_spent_chosen, amount_chosen, commission_chosen, cash_initial_chosen


def get_entry_compare(cash_initial_compare: float, cash_percentage: float, price_compare: float, transaction_cost: float):
...
def get_out_position_chosen(amount_chosen: float, price_chosen: float, transaction_cost: float):
...
def get_out_position_compare(amount_compare: float, price_compare: float, transaction_cost: float):
