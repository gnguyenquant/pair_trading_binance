...
def get_golden_data(chosen_symbol: str, compare_symbol: str, timeframe: str):
    chosen_symbol_df = pd.read_csv(
        'golden_set_{}_{}.csv'.format(chosen_symbol, timeframe))
    compare_symbol_df = pd.read_csv(
        'golden_set_{}_{}.csv'.format(compare_symbol, timeframe))
    return chosen_symbol_df, compare_symbol_df
...
def get_backtest_plot(chosen_symbol: str, compare_symbol: str, time_series: list, price_chosen:list,
                      price_compare: list, positions: list, zscore: list, critical_points: list):
    fig, (ax1, ax2, ax3) = plt.subplots(3, figsize=(15, 15))
...
def analysis_PnL(chosen_symbol: str, compare_symbol: str,time_series:list,PnL:list,risk_free_rate:float,fee_over_PnL:float):
    PnL_np=np.array(PnL)
    PnL_np=PnL_np-risk_free_rate
    PnL_cumsum=np.cumsum(PnL_np)
    PnL_mean=np.mean(PnL_np)
    PnL_std=np.std(PnL_np)
    annualized_Sharpe=PnL_mean/PnL_std*np.sqrt(24/2*365)
    index_end_max_drawdown=np.argmax(np.maximum.accumulate(PnL_cumsum)-PnL_cumsum)
    index_start_max_drawdown=np.argmax(PnL_cumsum[:index_end_max_drawdown])
    max_drawdown=PnL_cumsum[index_start_max_drawdown]-PnL_cumsum[index_end_max_drawdown]
    plt.figure(figsize=(16, 9))
    plt.title('Pair: {} {}. Sharpe ratio: {:.2f}. Maximal Drawdown: {:.2f}. Fee over PnL: {:.2f}'.format(chosen_symbol,compare_symbol,annualized_Sharpe,max_drawdown,fee_over_PnL))
    plt.xlabel('Cummulative PnL')
    plt.plot(time_series,PnL_cumsum,label='Cummulative PnL')
    plt.plot([time_series[index_start_max_drawdown],time_series[index_end_max_drawdown]],[PnL_cumsum[index_start_max_drawdown],PnL_cumsum[index_end_max_drawdown]],color='Red',label='Maximal drawdown')
    plt.legend()
